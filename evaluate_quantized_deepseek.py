...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Placement strategy for quantized path. Use 'auto' to shard across multiple GPUs with accelerate. "
             "If not specified, will auto-detect: use 'auto' if multiple GPUs available, otherwise 'none'.",
    )
...
def main():
    ...
    # Auto-detect device_map if not specified
    if args.device_map is None:
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            args.device_map = "auto"
            print(f"Detected {torch.cuda.device_count()} GPUs. Using device_map='auto' for multi-GPU support.")
        else:
            args.device_map = "none"
            if torch.cuda.is_available():
                print(f"Using single GPU (device_map='none').")
            else:
    ...
    if args.evaluate_bf16:
        ...
        # Use device_map for multi-GPU support in bf16 mode
        bf16_device_map = args.device_map if args.device_map == "auto" else ("auto" if args.device.startswith("cuda") and torch.cuda.device_count() > 1 else None)
        # Mirror deepseek_main.py behavior but use bfloat16 as requested
        model = DeepseekV2ForCausalLM.from_pretrained(
            args.model_path,
            config=config,
            device_map=bf16_device_map,
    ...
    # When using device_map="auto", the model is already distributed across GPUs
    # HFLM should use "cuda" as device to work with multi-GPU models
    hflm_device = "cuda" if args.device_map == "auto" and torch.cuda.is_available() else args.device
    lm = HFLM(
        pretrained=model,
        tokenizer=tokenizer,
