...
# OLMoE attention modules (standard QKV structure)
atten_modules = [
    "self_attn.q_proj",
    "self_attn.k_proj",
    "self_attn.v_proj",
...
# OLMoE expert modules - 64 experts, each with gate_proj, up_proj, down_proj
expert_modules = []
...
def get_model():
    ...
    model = OlmoeForCausalLM.from_pretrained(
        args.model, config=config, device_map='cuda', torch_dtype=torch.float16, trust_remote_code=True
    )
    print(model)
    # OLMoE model check - use model type from config instead of isinstance
    model_type = getattr(config, 'model_type', '').lower()
    if 'olmoe' not in model_type and 'olmo' not in model_type:
        logger.warning(f"Model type is {model_type}, expected olmoe/olmo model")
    model.seqlen = 2048
    return model


@torch.no_grad()
def olmoe_sequential(model, dataloader, dev, bit_config=None):
    print('Starting ...')
            ...
            for name in subset:
                ...
                if args.pack:
                    ...
                    if len(name_parts) == 2: # atten layer
                        ...
                        # OLMoE expert layer: mlp.experts.{expert_id}.{weight_name}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model", type=str, help="model to load; for example `allenai/OLMoE-1B-0924`."
    )
    parser.add_argument(
        "--wbits",
        type=str,
