"""
Example Python-only entrypoint to run OLMoE-1B-7B with test-time MoE rerouting
inside lm-evaluation-harness.

Usage:
    python run_olmoe_reroute.py --model allenai/OLMoE-1B-7B-0924 --tasks hellaswag arc_easy
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OLMoE rerouting eval via Python API")
    parser.add_argument(
        "--model",
        type=str,
        required=True,
        help="HF model id or local path (e.g., allenai/OLMoE-1B-7B-0924)",
    )
    parser.add_argument(
        "--tasks",
        nargs="+",
